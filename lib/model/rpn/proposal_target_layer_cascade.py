...
class _ProposalTargetLayer(nn.Module):
    ...
    def _sample_rois_pytorch(self, all_rois, gt_boxes, fg_rois_per_image,
        ...
        labels = gt_boxes[:, :, 4].contiguous().view(
        ...
        for i in range(batch_size):
            ...
            if fg_num_rois > 0 and bg_num_rois > 0:
                ...
                # See https://github.com/pytorch/pytorch/issues/1868
                # for more details.
                # use numpy instead.
                ...
                # rand_num = (torch.rand(
                #   bg_rois_per_this_image) * bg_num_rois).long().cuda()
                rand_num = np.floor(
                ...
                # sampling fg
                # rand_num = torch.floor(torch.rand(
                #   rois_per_image) * fg_num_rois).long().cuda()
                rand_num = np.floor(
                ...
                # sampling bg
                # rand_num = torch.floor(torch.rand(
                #   rois_per_image) * bg_num_rois).long().cuda()
                rand_num = np.floor(
                ...
                raise ValueError(
                    "bg_num_rois = 0 and fg_num_rois = 0, " + \
