                                         ...
                                         int roiPerImage)
{
   CUDA_KERNEL_LOOP(index, nthreads)
   {
       ...
       if(topLeftIsIn) inTopLeft = inputImages_data[inTopLeftAddress + cOut];
       if(topRightIsIn) inTopRight = inputImages_data[inTopRightAddress + cOut];
       if(bottomLeftIsIn) inBottomLeft = inputImages_data[inBottomLeftAddress + cOut];
       if(bottomRightIsIn) inBottomRight = inputImages_data[inBottomRightAddress + cOut];
                                         ...
                                         int roiPerImage)
{
  ...
  CUDA_KERNEL_LOOP(index, nthreads)
  {
      ...
      float gradOutValue = gradOutput_data[gradOutputAddress + cOut];
      // bool between(int value, int lowerBound, int upperBound)
      if(topLeftIsIn)
      {
         float inTopLeft = inputImages_data[inTopLeftAddress + cOut];
         topLeftDotProduct += inTopLeft * gradOutValue;
         if(!onlyGrid) atomicAdd(&gradInputImages_data[gradInputImagesTopLeftAddress + cOut], xWeightTopLeft * yWeightTopLeft * gradOutValue);
      }

      if(topRightIsIn)
      {
         float inTopRight = inputImages_data[inTopRightAddress + cOut];
         topRightDotProduct += inTopRight * gradOutValue;
         if(!onlyGrid) atomicAdd(&gradInputImages_data[gradInputImagesTopRightAddress + cOut], (1 - xWeightTopLeft) * yWeightTopLeft * gradOutValue);
      }

      if(bottomLeftIsIn)
      {
         float inBottomLeft = inputImages_data[inBottomLeftAddress + cOut];
         bottomLeftDotProduct += inBottomLeft * gradOutValue;
         if(!onlyGrid) atomicAdd(&gradInputImages_data[gradInputImagesBottomLeftAddress + cOut], xWeightTopLeft * (1 - yWeightTopLeft) * gradOutValue);
      }

      if(bottomRightIsIn)
      {
         float inBottomRight = inputImages_data[inBottomRightAddress + cOut];
         bottomRightDotProduct += inBottomRight * gradOutValue;
         if(!onlyGrid) atomicAdd(&gradInputImages_data[gradInputImagesBottomRightAddress + cOut], (1 - xWeightTopLeft) * (1 - yWeightTopLeft) * gradOutValue);
      }
  }
}
