  ...
  def __getitem__(self, index):
    ...
    if data_height > data_width:
        ...
        gt_boxes[:, 3].clamp_(0, self.trim_height)
            ...
            # if not ((gt_boxes[:, 1].sum() == 0 and gt_boxes[:, 3].sum() == 0) or 
            #         (gt_boxes[:, 1].mean() >= self.trim_height and gt_boxes[:, 3].mean() >= self.trim_height)):
            #     break
        ...
        gt_boxes[:, 2].clamp_(0, self.trim_width)
            ...
            # if not ((gt_boxes[:, 0].sum() == 0 and gt_boxes[:, 2].sum() == 0) or 
            #         (gt_boxes[:, 0].mean() >= self.trim_width and gt_boxes[:, 2].mean() >= self.trim_width)):
            #     break
